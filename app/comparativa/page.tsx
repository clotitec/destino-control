  ...
  const loadComparativa = async () => {
    ...
    try {
      ...
      const comps = Object.entries(METRICA_LABELS).map(([key, label]) => ({
        titulo: label,
        actual: sumar(actual, key),
        anterior: sumar(anterior, key),
      }))
      setComparaciones(comps)

      // POI ranking from tour_views metadata
      const poiMap: Record<string, { actual: number; anterior: number; daily: Record<string, number> }> = {}

      const processPOIs = (rows: MetricaRow[], field: 'actual' | 'anterior') => {
        rows.filter(r => r.metrica === 'tour_views').forEach(row => {
          ...
          if (porPoi) {
            Object.entries(porPoi).forEach(([poi, val]) => {
              ...
              if (field === 'actual') {
                poiMap[poi].daily[row.fecha] = (poiMap[poi].daily[row.fecha] || 0) + val
              }
            })
          }
        })
      ...
      const ranking = Object.entries(poiMap)
        .map(([poi, data]) => ({
          ...
          sparkline: Object.values(data.daily).slice(-14),
        }))
              ...
              <table className="w-full">
                ...
                <tbody>
                  {poiRanking.map((poi, i) => {
                    const delta = poi.anterior > 0
                      ? ((poi.actual - poi.anterior) / poi.anterior * 100)
                      : 0
                    return (
